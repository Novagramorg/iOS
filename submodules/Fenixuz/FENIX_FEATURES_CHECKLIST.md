...
**Done + Partial (qisman ham hisobga olinsa):** `[█████░░░░░] 46%`

> Hisob-kitob: 15 done + 6 partial + 24 missing + 1 out-of-scope = 46 ta. Out-of-scope (Kanal istoriyasi) upstream Telegram'da bor, shu sabab "qoldi" ro'yxatiga kirmaydi.
...
| 23 | ❌ | To'liq oq (white) rejim rangi o'zgartirildi | White theme color customization yo'q (`Brand/`, `Localization/`); 'white_theme_color' key yo'q | White theme color picker; brand color variant; UserDefaults'ga saqlash + light-theme'ga apply |
| 24 | ❌ | Chat ID ko'rsatish (top-right menu) | 'Copy Chat ID' action yo'q (ChatController context menu); 'show_chat_id' key yo'q | Chat header right-menu'ga 'Copy Chat ID'; peerId UIPasteboard'ga; Settings > Chat toggle |
| 25 | ❌ | Translate voice to chat | `SpeechToTextManager.swift` faqat voice→text, translation pipeline yo'q | STT'dan keyin tarjima hook (onTextUpdate callback yoki post-STT pipeline) |
...
| 35 | ❌ | Auto yuklab olishni boshqarish | FenixSettings'da auto-download toggle yo'q; 'auto_download' key yo'q | Settings toggle + media download path'ga hook (auto-download'ni gate qilish) |
...
| 42 | ❌ | Menu itemlarda haptic animatsiya | Haptic feedback kodi yo'q (UIFeedbackGenerator topilmadi); BUILD'da haptic dep yo'q | UIFeedbackGenerator(.selection/.impact): context menu, NavigationButtonComponent, ChatListCell long-press, Settings row |
...
| 46 | 🟡 | Secret chat + parol | `ChatPincodeManager.swift` + `ChatPincodeViewController.swift`; `NavigateToChatController.swift`; `ChatContextMenus.swift` | ~60% tayyor. Yo'q: (1) biometric (Face ID/Touch ID), (2) text/alphanumeric parol, (3) secret-chat'ga maxsus UI |
...
- [ ] **#46** Secret chat + parol — 🟡 partial (~60%, biometric yo'q)
...
- [ ] **#25** Translate voice to chat — ❌ missing
...
- [ ] **#23** To'liq oq (white) rejim rangi o'zgartirildi — ❌ missing
- [ ] **#35** Auto yuklab olishni boshqarish — ❌ missing
...
| 46 | Secret chat + parol | LocalAuthentication (Face ID/Touch ID); text-password rejim; secret-chat'ga maxsus branding |
...
| 23 | White rejim rangi | White theme color picker + brand variant + light-theme apply |
| 24 | Chat ID ko'rsatish | Right-menu 'Copy Chat ID' + peerId UIPasteboard + Settings toggle |
| 25 | Translate voice to chat | STT'dan keyin tarjima hook (onTextUpdate / post-STT) |
...
| 35 | Auto yuklab olish boshqaruv | Settings toggle + media download path hook |
...
| 42 | Menu haptic animatsiya | UIFeedbackGenerator: context menu, NavButton, ChatListCell, Settings row |
...
*Generated 2026-06-12 • Pro Messenger Imkoniyatlari.pdf • Fenixuz fork (Telegram-iOS)*
